...
const SAMPLE_QUESTIONS = ["자기소개 부탁드립니다.", "지원 동기를 말씀해주세요.", "본인의 강점은 무엇인가요?"];
...
export function InterviewSessionPage() {
  ...
  const messages = [
    { type: "question", text: SAMPLE_QUESTIONS[0], timestamp: new Date().toISOString() }
  ];
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 px-5 py-3">
        <div className="max-w-[390px] mx-auto flex items-center justify-between">
          ...
          <Button variant="danger" onClick={handleEnd} className="h-9 px-4 text-sm">
            종료
          </Button>
        </div>
      </div>
            ...
            <div key={idx} className={`flex ${msg.type === "answer" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[80%] rounded-2xl px-4 py-3 ${
                  msg.type === "question" ? "bg-white border border-gray-200" : "bg-primary text-white"
                }`}
              >
                <p className="text-sm mb-1">{msg.text}</p>
                <p className={`text-xs ${msg.type === "question" ? "text-gray-500" : "text-blue-100"}`}>
                  {msg.timestamp}
                </p>
              </div>
            </div>
